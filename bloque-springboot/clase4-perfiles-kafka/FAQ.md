# FAQ - Clase 4: Perfiles, Configuración y Apache Kafka

Respuestas a preguntas frecuentes sobre perfiles de Spring Boot, Actuator, variables de entorno y Apache Kafka.

---

## Spring Boot Profiles

### ¿Por qué usar perfiles en lugar de múltiples application.yml?

**Ventaja de perfiles**:

- Un solo JAR funciona en todos los entornos
- Cambias perfil con variable de entorno o parámetro
- Evitas compilar diferente binario por entorno

**Sin perfiles**:

- Necesitas mantener múltiples archivos de configuración separados
- Riesgo de desplegar configuración incorrecta

### ¿Puedo tener más de un perfil activo simultáneamente?

Sí, puedes activar múltiples perfiles separados por coma:

```bash
mvn spring-boot:run -Dspring-boot.run.profiles=dev,local,debug
```

Spring Boot carga en orden:

1. `application.yml`
2. `application-dev.yml`
3. `application-local.yml`
4. `application-debug.yml`

Propiedades de perfiles posteriores sobrescriben las anteriores.

### ¿Dónde se define el perfil por defecto si no especifico ninguno?
...
Si no especificas, Spring Boot usa el perfil `default`.

### ¿Cómo sé qué perfil está activo?

Al iniciar, Spring Boot imprime:

```
The following 1 profile is active: "dev"
```

También puedes consultar mediante código:
...
### ¿Es seguro exponer /actuator/env en producción?

**NO**, porque `/actuator/env` muestra:
...
NO incluir `env` en `management.endpoints.web.exposure.include` en producción:
...
O proteger con Spring Security:
...
### ¿Por qué /actuator/health muestra solo {"status":"UP"} sin detalles?

Por defecto, `show-details` está en `never`. Cambiar a:
...
### ¿Cómo agregar mis propias métricas?

Inyectar `MeterRegistry` y crear métricas:
...
### ¿Qué otros health indicators puedo agregar?

Crear custom health indicator:
...
public class ExternalServiceHealthIndicator implements HealthIndicator {
    ...
    public Health health() {
        try {
            restTemplate.getForEntity("https://external-api.com/health", String.class);
...
### ¿Por qué usar variables de entorno en lugar de hardcodear?

**12-Factor App** (principio III: Config):

- **Seguridad**: Credenciales NO van a Git
- **Portabilidad**: Mismo JAR en dev/staging/prod
- **Flexibilidad**: Cambiar sin recompilar
- **Cloud-ready**: Docker, Kubernetes, Heroku

### ¿Qué pasa si olvido definir una variable de entorno crítica?

Spring Boot usa el valor por defecto (fallback).
...
Si `DB_PASSWORD` NO está definida, usa `postgres`.

**Problema**: En producción podrías usar credenciales inseguras.
...
### ¿Puedo usar variables de entorno en Spring Boot sin Docker?

Sí, definir antes de ejecutar:
...
### ¿Cómo gestionar múltiples archivos .env para diferentes entornos?
...
**Herramienta**: [direnv](https://direnv.net/) carga automáticamente según directorio.
